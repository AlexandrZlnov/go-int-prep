 ...
 * [task11](tasks/chan/task11/): примеры 3-х deadlock‑кейсов с каналами
 * [task12](tasks/chan/task12/): реализовать кейсы на порядок выполнения goroutine, FIFO-гарантии каналов и happens-before
 * …
</details>
 ...
 * [task3](tasks/concurrency/task3/): анализ проблем в самописном WaitGroup и исправление
 ...
 * [task8](tasks/concurrency/task8/): реализовать worker pool для конкурентной обработки списка задач с ограничением количества одновременно работающих goroutine, корректной синхронизацией завершения и безопасной записью результатов без data race.
 * [task9](tasks/concurrency/task9/): реализовать безопасную синхронизацию goroutine и устранить race condition с обеспечением happens-before отношений через channels, mutex и atomic операции.
 * [task10](tasks/concurrency/task10/): реализовать worker pool для конкурентной обработки задач через channels с фиксированным количеством workers, отдельным results channel и корректным graceful shutdown без утечек goroutine и data race.
 * …
</details>
...
* [task1](tasks/gr/task1/): реализовать параллельное выполнение HTTP-запросов к двум URL (Google и Wildberries) и вывести HTTP-коды ответов для каждого запроса.
* [task2](tasks/gr/task2/): проанализировать порядок выполнения горутин и каналов при запуске нескольких worker’ов и определить фактическое время ожидания результата
 * [task3](tasks/gr/task3/): исправить конкурентный код с использованием WaitGroup и unbuffered channel так, чтобы корректно передавались результаты из goroutine, не происходило утечек горутин и программа завершалась без deadlock.
 * [task4](tasks/gr/task4/): организовать параллельное выполнение группы goroutine с управлением жизненным циклом через context.Context, поддержкой timeout/cancellation и безопасным завершением всех concurrent операций.
 * …
</details>
 ...
 * [task3](tasks/other/task3/): решение задачи с балансировкой банковских вызовов и обработкой ошибок
